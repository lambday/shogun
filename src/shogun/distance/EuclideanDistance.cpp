...
bool CEuclideanDistance::init(CFeatures* l, CFeatures* r)
{
	...
	REQUIRE(casted_l->get_dim_feature_space()==casted_r->get_dim_feature_space(),
		"Number of dimension mismatch (l:%d vs. r:%d) in feature space!\n",
...
float64_t CEuclideanDistance::distance_upper_bounded(int32_t idx_a, int32_t idx_b, float64_t upper_bound)
{
	...
	float64_t* avec=casted_lhs->get_feature_vector(idx_a, alen, afree);
	float64_t* bvec=casted_rhs->get_feature_vector(idx_b, blen, bfree);

	REQUIRE(alen==blen, "The vector lengths are not equal!\n");
	...
	casted_lhs->free_feature_vector(avec, idx_a, afree);
	casted_rhs->free_feature_vector(bvec, idx_b, bfree);
